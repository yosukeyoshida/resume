...
* バンドルカードのAPIサーバにおけるSaaS接続部分の内製化への切り替えに伴う改修

##### 実績
* 既存のコードからカードまわりのすべての処理を洗い出しシーケンス図, 状態遷移図の作成
* カード発行やチャージ, 決済通知等を担うAPIサーバの開発
    * アプリと通信するバンドルカードのAPIサーバ(以下バンドルAPI)からリクエストを受け以下の処理を行う
...
* バンドルAPIの改修
    ...
    * サーバ側同様OpenAPIにてAPIクライアントのHTTPリクエスト/レスポンスの自動生成
...
* 機械学習モデルの開発
    * サービス開始から1年近くはルールベースの改善によってKPIを伸ばし続けてきたがいくつか課題が出始めた
        ...
        3. KPIの成長が鈍化 / 事業の状況に合わせた数値のコントロールが困難
            * 複数のKPIのトレードオフ(返済率, 売上)を状況に応じて柔軟に調整することが難しい
    * 機械学習で解くべき課題の定義, 評価指標, モデル及び推論APIの構築, バンドルAPIとのインテグレーションまで2ヶ月弱でプロダクションに投入、上記課題に対しては
        ...
        3. KPIの成長が鈍化 / 事業の状況に合わせた数値のコントロールが困難
            * 機械学習モデルからの予測の閾値を調整することで、事業の状況にあわせた各種KPIのコントロールが容易になった
            * 機械学習モデル投入から10ヶ月, サービス開始から2年で当初目標としていたKPIに到達
* 推論APIサーバ, データパイプラインの構築
...
* 外部の提携企業とのファイル連携/バッチ処理
    * ファイル連携の要件定義, 業務設計, テストやスケジュール管理といったPM業務からファイル取込み/出力バッチ処理の実装まで担当
    * 特にこの開発に限った話ではないですがバッチ処理は冪等であること、リカバリが容易/不要であることを重視して設計
...
* 推論APIサーバをPythonで構築, 記事作成時にカテゴリを推論し保存

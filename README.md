...
#### 決済システム内製化
##### 課題/背景
* サービス開始からカード発行, 決済, 残高管理といった主要な機能は外部のSaaSを利用していたが、自社でコントロールすることのできない仕様や障害の発生、トランザクションの増加に伴うコスト増が問題となってきていた
...
* バンドルAPIの後払いチャージ機能の開発
    ...
    * 返済期限超過による機能制限 / 督促(email, sms, push, tel)
    * 複数アカウント等不正検知
* 機械学習モデルの開発
    * サービス開始から1年近くはルールベースの改善によってKPIを伸ばし続けてきたがいくつか課題が出始めた
        * ルールベースのロジックが複雑化
            * 保守性の低下やバグの温床
        * ユーザごとの柔軟なスコアリングが困難 / 活用しきれていないデータ
            * ハードコーディングされたロジックでは離散的で画一的な評価になってしまう
            * よりユーザの行動に即した多種多様なデータを反映したいがルールベースではそのようなデータを扱うことが困難
        * KPIの成長が鈍化 / 事業の状況に合わせた数値のコントロールが困難
            * 複数のKPIのトレードオフ(返済率, 売上)を状況に応じて調整することが難しい
    * 機械学習で解くべき課題の定義, 評価指標, モデル及び推論APIの構築, バンドルAPIとのインテグレーションまで2ヶ月弱でプロダクションに投入。上記課題に対しては、
        * ルールベースのロジックの複雑化
            * 極力ルールベースから機械学習モデルに判断を寄せることでロジックを簡素化
        * ユーザごとの柔軟なスコアリングが困難 / 活用しきれていないデータ
            * ルールベースでは扱うことが困難であったデータでも機械学習モデルの特徴量に組み込むことで、よりユーザの行動に即した評価が実現
        * KPIの成長が鈍化 / 事業の状況に合わせた数値のコントロールが困難
            * 機械学習モデルの出力の閾値を調整することで事業の状況にあわせた数値のコントロールが容易になった

...
##### 担当
* カード発行やチャージ, 決済通知等を担うAPIサーバの開発/DB設計
...
##### 工夫した点
* REST APIのスキーマ定義をリリース時はJSON Schemaを採用し、リリース後しばらくしてからOpenAPIへ移行
    * 技術選定時点ではJSON Schemaと比較してOpenAPI(v2)には以下のようなデメリットがあった
        * 決済通知などのWebhookを記述することができない
        * oneOf, anyOfのような表現が使えない、バリデーションができない
        * JSON Schemaのdefinitionsのようなプロパティの定義ができない
    * スキーマからバリデーションやリクエスト/レスポンスの構造体を自動生成することは外せない要件と考えていたので上記の理由からJSON Schemaを採用
    * JSONのバリデーションにはgojsonschemaを採用したが、gojsonschemaに対してJSON Schemaをそのまま使うことはできなかった
        * JSON Schemaをパースしてエンドポイント毎のバリデーション定義を自動生成するツールをPythonで自作
    * その後、OpenAPI v3では上記の問題を解消できるような仕様(Callback, Components, oneOf)が追加されたこと、v3対応のバリデーション等の周辺ライブラリが充実してきたこともありOpenAPIに移行した
* 冪等性の考慮
...
* 機械学習モデルの開発
    ...
    * 機械学習で解くべき課題の定義, 評価指標, モデル及び推論APIの構築, バンドルAPIとのインテグレーションまで2ヶ月弱でプロダクションに投入、上記課題に対しては
        1. ルールベースのロジックの複雑化
            * 機械学習モデルに判断を寄せることでルールベースへの依存を低減
        2. ユーザごとの柔軟なスコアリングが困難 / 活用しきれていないデータ
            * ルールベースでは扱うことが困難であったデータも機械学習モデルの特徴量に組み込むことが可能となり画一的でないユーザ毎の評価が可能になった
        3. KPIの成長が鈍化 / 事業の状況に合わせたKPIのコントロールが困難
            * 機械学習モデルを用いるメリットのひとつとして予測を単一の実数値に落とし込むことができる点にあり、意思決定に用いる予測の閾値を調整することで返済率や売上といったKPIのコントロールが容易になった
...
##### 工夫した点
* ぎりぎりまでルールベースで成長させる
* 1人だったのでMLリリース時は最低限のMLOps マネージドに寄せる
* 先にアウトプットまで作ってしまう、評価指標
* 前処理は極力SQLでGoにテストを寄せる
...
##### 実績
* 検索基盤をElasticsearchで構築
* 形態素解析を用いたトークナイザの追加、タイトル, 本文, タグ等の検索ウェイトの調整、画像のリンク切れ等記事のクォリティを検索結果に反映するなどの改善を実施
    * 離脱率: 25.28% -> 15.40%
    * 検索後記事に辿り着くまでのページ数: 5.25 -> 2.39
* 形態素解析の辞書, シノニムの整備
    * インテリアや雑貨などは次々と流行の新語が生まれてくるので継続的な辞書のメンテナンスが必要
        * 誰でも辞書のメンテナンスができるように管理画面を用意
        * 社内ユーザであれば記事検索結果で検索ワードと形態素解析の結果を確認できるようにし、正しく認識されない単語をすぐに登録できるようにした
* 記事更新時にSQSを用いて非同期にインデックスを更新することで即座に検索への反映を実現
* 記事検索以外へ応用した機能開発
    * 関連記事などのコンテンツベースでのレコメンデーションによる回遊率の改善
    * 形態素解析, TF-IDFを利用した自動タグ付け
    * カテゴリ毎のトピックや関連タグなどの導線強化
...
##### 工夫した点
* スケールさせなかった
* 直接カテゴリを修正するのではなく、新カテゴリを修正していった
* スマホで修正できるようにして通勤時間とかポチポチしてた
* 機械学習と併用していった
...
#### 機械学習を用いた記事の自動カテゴリ分類
##### 背景/課題
* 上記サイト構造のリニューアルに伴い、カテゴリが細分化され記事の執筆者がどのカテゴリを設定すればよいのか判断が難しくなってしまった

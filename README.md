...
* カード発行やチャージ, 決済通知等を担うAPIサーバの開発
    * 技術選定から設計/開発まですべてを担当
    * アプリと通信するバンドルカードのAPIサーバ(以下バンドルAPI)からリクエストを受け処理を行う
...
##### 担当
* バンドルカードのAPIサーバの後払い機能開発
* 機械学習モデルの開発
* 推論APIサーバ, データパイプラインの構築
* 予測性能とKPIのモニタリング, 継続的な改善

##### 実績
* バンドルカードのAPIサーバの後払い機能の設計/開発のすべてを担当
    * 認証 (メール, SMS)
    * チャージ / 支払い (コンビニ, ネット銀行, ペイジー etc)
    * ユーザ毎の申込み可能額の算出
    * 機械学習モデルの予測とのインテグレーション
    * 返済期限超過による機能制限
    * 督促(email, sms, push, tel)
    * 複数アカウント等不正利用の検出
* 機械学習モデルの開発
    * 教師あり/二値分類
    * ルールベースだけでは活用しきれなかったデータの活用による精度向上
    * 機械学習モデルの予測の閾値を調整することで複数のKPIのトレードオフの調整を容易にした
    * 複雑化したルールベースをモデルに寄せることでシンプルにした
* 機械学習パイプラインの構築

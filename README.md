...
* カード発行やチャージ, 決済通知等を担うAPIサーバの開発
    * アプリと通信するバンドルカードのAPIサーバ(以下バンドルAPI)からリクエストを受け以下の処理を行う
        ...
        * カードのステータス更新
    * 決済時にバンドルAPIに決済情報を通知
        * Visaの電文を処理するサーバとジョブキューを介して非同期に決済情報を連携し、バンドルAPIにwebhookで通知を行う
    * OpenAPIによるスキーマ駆動開発
...
* バンドルAPIの後払いチャージ機能の開発
    ...
    * 複数アカウント等不正決済情報を連携し、バンドルAPIにwebhookで通知を行う
* 機械学習モデルの開発
    * サービス開始から1年近くはルールベースの改善によってKPIを伸ばし続けてきたがそれだけでは解決できない課題が出始めた
        * ロジックの複雑化
        * ユーザごとの柔軟なスコアリング
        * 活用しきれていないデータ
        * 事業の状況に合わせたKPIのトレードオフの調整
        * KPIの成長の鈍化
    * 機械学習で解くべき課題の定義, 評価指標, モデル及び推論APIの構築, バンドルAPIとのインテグレーションまで2ヶ月弱でプロダクションに投入。上記課題に対しては、
        * ロジックの複雑化
            * 極力機械学習モデルの判断に寄せることでルールベースのロジックを簡素化
        * ユーザごとの柔軟なスコアリング / 活用しきれていないデータ
            * ルールベースだけでは組み込むことが困難であったデータを特徴量に追加することで多角的な評価の実現
            * ハードコーディングされた離散的で画一的な評価でなく、ユーザの行動に即した連続的でなめらかな評価の実現
        * 事業の状況に合わせたKPIのトレードオフの調整
            * 機械学習モデルの出力の閾値を調整することで事業の状況にあわせた数値のコントロールの簡易化
        * KPIの成長の鈍化

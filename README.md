...
##### 担当
* 検索基盤の構築とそれを用いたバックエンドの開発

##### 実績
* 検索基盤をElasticsearchで構築
    * 形態素解析を用いたトークナイザの追加や検索項目のウェイトの調整などのチューニングによる改善
        * 離脱率: 25.28% -> 15.40%
        * 検索後記事に辿り着くまでのページ数: 5.25 -> 2.39
* 形態素解析の辞書, シノニムの整備
* 記事更新時にSQSを用いて非同期にインデックスを更新することで即座に検索へ反映
* 記事検索以外へ応用した機能開発
...
##### 工夫した点
* 検索に記事の内容だけではなく画像のリンク切れや文字数のといった記事のクォリティを反映
* インテリアや雑貨などは次々と流行りの新語が生まれてくるため継続的な辞書のメンテナンスが必要
    * 社内ユーザであれば誰でも簡単に辞書が更新できるようにした
        * 辞書更新用の管理画面を用意
        * 記事検索結果で検索ワードと形態素解析の結果を確認できるようにし、正しく認識されない単語をすぐに登録できるようにした
...
##### 工夫した点
* 直接カテゴリを修正してしまうとSEOなどへの影響が大きいので新カテゴリは既存のカテゴリとは別にカラムを設けて水面下で進めていった
* カテゴリの修正作業は関わる人が多いと基準がぶれてしまう恐れがあったので少人数で実施
* ある程度新カテゴリが固まり記事のカテゴリ設定ができたタイミングで以下に述べる機械学習と併用してカテゴリの修正作業を進めていくことでラベリングの工数を削減した

...
* カード発行, 残高の加算/減算, 決済通知等を担うマイクロサービスを新規に設計/開発
    * REST APIサーバ
        * バンドルカードのAPIサーバ(以下バンドルAPI)からリクエストを受けて以下の処理を行う。残高を動かす処理はジョブキューを使い非同期処理でバンドルAPIへ通知を行う。
            * カード発行/再発行
            * チャージ
            * 手数料徴収
        * Visaと通信し決済処理をするサーバとジョブキューを介して非同期処理でバンドルAPIへ決済通知を行う
        * 冪等になるようにした
        * gojsonschemaを使っている。スキーマはjson hyper schema
            * json hyper schemaをparseしてgojsonschemaに合わせたバリデーション定義を自動生成するやつを作った
                * https://github.com/kanmu/jshschema-gojsonschema-codegen
    * データベース設計
        * 基本的にすべてテーブルはinsert
        * お金まわりもカードの履歴も
        * 残高は都度計算
